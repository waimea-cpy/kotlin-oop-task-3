...
class Steps() {
    ...
    fun show() {
        ...
        for ((i, gnome) in steps.withIndex()) {
            print("${i+1}: ")
            if (gnome != null) print(gnome.name)
            println()
        }
    }
    ...
    fun placeGnome(step: Int, gnome: Gnome) {
        println("Placing ${gnome.name} on step $step...")
        if (step in 1..STEPCOUNT) {
            val stepNum = stepNumOfGnome(gnome)
            if (stepNum != 0) steps[stepNum - 1] = null
            steps[step - 1] = gnome
        }
    }
    ...
    fun gnomeOnStep(stepNum: Int): Gnome? {
        if (stepNum in 1..STEPCOUNT) return steps[stepNum-1]
        return null
    }
    ...
    fun clearStep(step: Int) {
        println("Clearing step $step...")
        if (step in 1..STEPCOUNT) steps[step-1] = null
    }
    ...
    fun removeGnome(gnome: Gnome) {
        println("Removing gnome ${gnome.name}...")
        val stepNum = stepNumOfGnome(gnome)
        if (stepNum != 0) steps[stepNum-1] = null
    }
}
